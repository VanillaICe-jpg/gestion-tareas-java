...
public class ConsoleView {
    ...
    private void updateTask() {
        ...
        LocalDate dueDate = dateInput.isEmpty() ? task.getDueDate() : LocalDate.parse(dateInput);

        System.out.print("New Priority (HIGH, MEDIUM, LOW) [" + task.getPriority() + "]: ");
        String prioInput = scanner.nextLine();
        Priority priority = prioInput.isEmpty() ? task.getPriority() : Priority.valueOf(prioInput.toUpperCase());

        System.out.print("New Status (PENDING, IN_PROGRESS, COMPLETED) [" + task.getStatus() + "]: ");
        String statInput = scanner.nextLine();
        Status status = statInput.isEmpty() ? task.getStatus() : Status.valueOf(statInput.toUpperCase());

        boolean success = controller.updateTask(id, title, description, dueDate, priority, status);
